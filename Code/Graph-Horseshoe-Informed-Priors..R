...
## --- 1a.  Build an igraph object --------------------------------------
g <- graph_from_adjacency_matrix(W_obs,
                                 mode     = "undirected",
                                 weighted = TRUE,
                                 diag     = FALSE)

## (optional) drop isolates / threshold tiny edges
g <- delete_vertices(g, degree(g) == 0)
## current weights
w <- igraph::E(g)$weight
## choose any threshold you like: median, 75-th pct, etc.
cutoff <- quantile(w, 0.25)        # median

## drop the light edges
g <- igraph::delete_edges(g, igraph::E(g)[ w < cutoff ])


## --- 1b.  Find communities  -------------------------------------------
wc   <- cluster_louvain(g, weights = igraph::E(g)$weight)      # Louvain
comm <- membership(wc)                                 # vector length p

# If you prefer Leiden (crisper, needs the 'leiden' pkg):
# wc   <- leiden::leiden_find_partition(g, edge_weights = E(g)$weight)
# comm <- wc$membership
...
initialize_sampler <- function(p, n, X, y) {
  ...
  zeta2 <- 0.1#1
  nu_shape <- 0.5
  nu_rate  <-  0.04 #5      #Main one             # mean E[zeta2] ≈ 0.1
  #nu_rate  <- 1                   # E[ζ²] now ≳1
  a_tau <- 2 #1
  ...
  kappa_mat  <- ((q_mat - 1) / alpha_mat) * W_obs # Main one
  diag(kappa_mat) <- 1

  ################## High confidence Edges #########################
  p <- nrow(W_obs)

  # TRUE where i<j and both vertices are in the same detected community
  in_block <- outer(comm, comm, `==`)      # p × p logical
  in_block[ lower.tri(in_block, diag = TRUE) ] <- FALSE  # keep upper-tri only

  # TRUE for edges that connect *different* communities
  cross_block <- upper.tri(in_block) & !in_block

  # ------------- baseline (weakly-informative) --------------------------
  ...
  # ------------- strengthen *within* detected clusters ------------------
  alpha_in  <- 5                              # larger α  ⇒ sharper slab
  kappa_in  <- 0.1                            # shrink κ  ⇒ larger mean

  alpha_mat[in_block]  <- alpha_in
  kappa_mat[in_block]  <- kappa_in * kappa_mat[in_block]

  # ------------- soften *cross* edges (optional) ------------------------
  alpha_out <- 1.0                            # heavier tails
  kappa_out <- 2.0                            # push mean toward zero

  alpha_mat[cross_block] <- alpha_out
  kappa_mat[cross_block] <- kappa_out * kappa_mat[cross_block]
...
sample_tau2 <- function(a_tau, b_tau, omega, psi) {
  ...
  sum_w <- sum(omega[upper.tri(omega)])       
